...
## `/more-apps/` — 앱 안 「모두소프트의 다른 앱」 목록

카세이브·뭐무 설정 화면(패키지 `Chrisbaba/modusoft_more_apps`)이 `https://modusoft.kr/more-apps/apps.json` 을
받아 가서 그린다. 사람이 보는 페이지가 아니라 **앱이 읽는 데이터**다.
...
- 아이콘 `more-apps/icons/{id}.png` 는 각 앱 저장소의 1024 원본 아이콘을 192px 로 줄인 정사각형이다
  (모서리는 앱이 둥글린다). 경로가 패키지 번들 목록에 박혀 있으므로 **이름을 바꾸지 않는다**.
- 점점 앱은 네트워크를 쓰지 않으므로 이 목록을 읽지 않는다(설정의 링크로 이 사이트 홈을 연다).
